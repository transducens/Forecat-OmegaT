...
public class RankerNeuralNetwork extends RankerPressureBasic {
	...
	public List<SuggestionsOutput> rankerService(SuggestionsInput rankinp, List<SuggestionsOutput> input) {
		...
		features[2] = ((float) rankinp.getFixedPrefixCharLength()) / ((float) rankinp.getSourceCharLength());
		// Target length ratio in words
		features[3] = ((float) rankinp.getFixedPrefixWordLength()) / ((float) rankinp.getSourceWordLength());
		...
		for (SuggestionsOutput so : input) {
			...
			features[14] = ((float) (1 + features[0]) / (float) (1 + rankinp.getSourceCharLength()))
					- ((float) (1 + features[4]) / (float) (1 + rankinp.getSourceCharLength()));
			// Sourcepos/sourcelen - targetpos/targetlen in words
			features[15] = ((float) (1 + features[1]) / (float) (1 + rankinp.getSourceWordLength()))
					- ((float) (1 + features[5]) / (float) (1 + rankinp.getSourceCharLength()));
			// Sourcepos/targetpos in chars
			features[16] = ((float) (1 + features[0])) / ((float) (1 + features[4]));
			// Sourcepos/targetpos in words
			features[17] = ((float) (1 + features[1])) / ((float) (1 + features[5]));
			...
			if (((diffAvg - .5 * diffDev) <= features[13]) && (features[13] <= (diffAvg + .5 * diffDev))) {
				features[63] = 1;
			} else if (((diffAvg - 1 * diffDev) <= features[13]) && (features[13] <= (diffAvg + 1 * diffDev))) {
				features[64] = 1;
			} else if (((diffAvg - 2 * diffDev) <= features[13]) && (features[13] <= (diffAvg + 3 * diffDev))) {
				features[65] = 1;
			} else {
				features[66] = 1;
			}
			...
			if (((ratioAvg - .5 * ratioDev) <= features[17]) && (features[17] <= (ratioAvg + .5 * ratioDev))) {
				features[67] = 1;
			} else if (((ratioAvg - 1 * ratioDev) <= features[17]) && (features[17] <= (ratioAvg + 1 * ratioDev))) {
				features[68] = 1;
			} else if (((ratioAvg - 2 * ratioDev) <= features[17]) && (features[17] <= (ratioAvg + 3 * ratioDev))) {
				features[69] = 1;
			} else {
				features[70] = 1;
			}
