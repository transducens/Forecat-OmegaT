...
//				SessionLogPluginAccesser.GenericEvent("GENERATED_SUGGESTION",
//						"TRANSLATED", s.getId() + "," + s.getPosition() + ","
//								+ s.getCharPosition() + ","
//								+ s.getSourceSegmentText().split(" ").length,
//						entry.getKey());
			}
		}
	}

	protected void logSuggestions(SessionShared session,
			SuggestionsInput inputSuggestions,
			List<SuggestionsOutput> outputSuggestions) {
//		for (SuggestionsOutput so : outputSuggestions) {
//			SessionLogPluginAccesser.GenericEvent("OFFERED_SUGGESTION",
//					"OFFERED",
//					so.getPosition() + "," + so.getSuggestionFeasibility()
//							+ "," + inputSuggestions.getPosition() + ","
//							+ inputSuggestions.getPrefixText(), so.getId());
//		}

	}

	protected void logSelect(SessionShared session,
			SelectionInput inputSelection, SelectionOutput outputSelection) {
//		SessionLogPluginAccesser.GenericEvent("USED_SUGGESTION", "USED", "",
